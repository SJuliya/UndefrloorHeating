...
const calcResultWrapper = document.querySelector('.js-result');
...
calcForm.addEventListener('input', () => {
    btnSubmit.disabled = !(calcForm.width.value > 0 && calcForm.height.value > 0);
});

calcForm.addEventListener('submit', (event) => {
    event.preventDefault();

    if(calcForm.width.value > 0 && calcForm.height.value > 0) {
        const square = calcForm.width.value * calcForm.height.value;
...
validator
    .addField('#name', [
        ...
        },
        {
            rule: 'minLength',
            value: 3,
            errorMessage: 'Не меньше 3-х символов'
        },
        {
            rule: 'maxLength',
            value: 30,
...
validator.onSuccess((event) => {
    ...
    .then((data) => {
        form.reset();
        alert(`Спасибо за заказ (№ ${data.id}), мы с вами свяжемся!`)
    });
});